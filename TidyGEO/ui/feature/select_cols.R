tabPanel(title = FILTER_ICON, value = "2",
         ...
         radioButtons(inputId = "feature_filter_option", label = div("Please select an option:", help_link("feature", "filter_help")),
         ...
         conditionalPanel(condition = "input.feature_filter_option == 'preset_filters'",
                          ...
                          checkboxGroupInput(inputId = "feature_download_data_filter", label = NULL,
         ...
         conditionalPanel(condition = "input.feature_filter_option == 'column_filters'",
                          div(tags$b("By column name"),
                              help_button("This will drop unselected columns from the table.")),
                          tags$i("Keep the following columns..."),
                          checkboxInput(inputId = "feature_select_all_columns", label = tags$i("Select all"), value = TRUE),
                          #checkboxGroupInput(inputId = "varsToKeep", label = NULL)
                          uiOutput("feature_display_vars_to_keep")
         ),
         div(
           primary_button(id = "feature_evaluate_filters", label = div(FILTER_ICON, "Filter columns")),
           undo_button("feature_undo_select")
         ),
