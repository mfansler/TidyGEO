...
observe({
  updateCheckboxGroupInput(
    session, 'cols_to_split', choices = colnames(clinical_vals[[dataname("clinical")]]),
    selected = if (input$select_all_split) colnames(clinical_vals[[dataname("clinical")]])
  )
})
...
observeEvent(input$split_pairs, ({
  if (!is.null(clinical_vals[[dataname("clinical")]])) {
    ...
    #clinical_vals[[dataname("clinical")]] <- withProgress(extractColNames(clinical_vals[[dataname("clinical")]],
