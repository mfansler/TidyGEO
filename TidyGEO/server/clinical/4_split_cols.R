...
observeEvent(input$split_cols, ({
  if (!is.null(clinical_vals$clinical_data)) {
    ...
    save_lines(commentify("extract values from columns with delimiter"), "clinical", "body")
    add_function("splitCombinedVars", "clinical")
    save_lines(paste0("cols_to_divide <- ", format_string(input$colsToDivide)), "clinical", "body")
    save_lines(c(paste0("divide_delimiter <- ", format_string(input$divide_delimiter)),
                                       paste0("divide_regex <- ", format_string(input$split_cols_w_regex)),
                                       "clinical_data <- splitCombinedVars(clinical_data, cols_to_divide, divide_delimiter, divide_regex)"), 
                                     "clinical", "body")
  }
